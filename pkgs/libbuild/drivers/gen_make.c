...
static int gen_pkg(const pkg_t *pkg, print_dst_ex_t dst)
{
	make_t make = {0};
	make_init(&make, 32, 32, 2, 32, ALLOC_STD);

	make_var_add_val(&make,
			 make_add_act(&make, make_create_var(&make, STRV("PKGNAME"), MAKE_VAR_INST, NULL)),
			 MSTR(strn(pkg->name.data, pkg->name.len, pkg->name.len + 1)));
...
static int gen_make(const gen_driver_t *drv, const proj_t *proj)
{
	...
	if (proj->is_pkg) {
		...
		make_add_act(&make, make_create_phony(&make));

		make_rule_t test = make_add_act(&make, make_create_rule(&make, MRULE(MSTR(STR("test"))), 0));
		make_rule_add_depend(&make, test, MRULEACT(MSTR(strc(proj->pkg.name.data, proj->pkg.name.len)), STR("/test")));

		make_rule_t cov = make_add_act(&make, make_create_rule(&make, MRULE(MSTR(STR("coverage"))), 0));
		make_rule_add_depend(&make, cov, MRULE(MSTR(STR("test"))));

		make_rule_add_act(&make,
				  cov,
				  make_create_cmd(&make,
						  MCMD(STR("lcov -q -c -o $(BUILDDIR)/bin/lcov.info -d $(INTDIR)/libbuild/src -d "
