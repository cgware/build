...
void *test_lib_open(print_dst_ex_t dst, const char *path, const char *mode)
{
	CSTART;
	...
	switch (file) {
	case 0:
		EXPECT_STR(path, "./test/lib/pkg.mk");
		break;
	case 1:
		EXPECT_STR(path, "./test/lib/Makefile");
		break;
	default:
		EXPECT_FAIL("Unexpected file: %s\n", path);
		break;
...
int test_lib_close(print_dst_ex_t dst)
{
	CSTART;
