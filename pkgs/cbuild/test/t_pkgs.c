...
TEST(pkgs_get_build_order)
{
	...
	pkg = pkgs_add_pkg(&pkgs, STRV("0"), NULL);
	pkg_add_target(pkg, &targets, STRV("0"), &target);

	targets_add_dep(&targets, target, STRV("1"));

	pkg = pkgs_add_pkg(&pkgs, STRV("1"), NULL);
	pkg_add_target(pkg, &targets, STRV("1"), &target);

	pkg = pkgs_add_pkg(&pkgs, STRV("2"), NULL);
	pkg_add_target(pkg, &targets, STRV("2"), &target);
...
TEST(pkgs_print)
{
	...
	char buf[256] = {0};
	EXPECT_EQ(pkgs_print(&pkgs, &targets, PRINT_DST_BUF(buf, sizeof(buf), 0)), 102);
	EXPECT_STR(buf,
		   "[package]\n"
		   "ID: 0\n"
		   "DIR: \n"
