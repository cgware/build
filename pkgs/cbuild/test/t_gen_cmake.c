...
TEST(gen_cmake_proj_build_dir)
{
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "\t\t\tlcov -q -c -o ${DIR_TMP_COV}lcov.info -d ${CMAKE_BINARY_DIR} --ignore-errors unused --exclude \\\"*/test/*\\\" "
		    "--exclude "
		    "\\\"*/example/*\\\" --exclude \\\"*/tmp/*\\\"\\;\n"
...
TEST(gen_cmake_proj_ext)
{
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "if(NOT EXISTS ${DIR_TMP_DL}${PKG_URI_FILE})\n"
		    "\tfile(DOWNLOAD ${PKG_URI} ${DIR_TMP_DL}${PKG_URI_FILE}\n"
		    "\t\tSHOW_PROGRESS\n"
		    ")\n"
		    "endif()\n"
		    "file(ARCHIVE_EXTRACT INPUT \"${DIR_TMP_DL}${PKG_URI_FILE}\" DESTINATION \"${DIR_TMP_EXT_PKG_SRC}\")\n"
...
TEST(gen_cmake_pkg_drv)
{
	START;

	t_gen_common_t com = {0};
	EXPECT_EQ(t_gen_pkg_drv(&com, STRV("C")), 0);
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "list(FILTER ${PN}_${TN}_src EXCLUDE REGEX \"drivers/(windows|linux)/.*\\\\.c$\")\n"
		    "file(GLOB_RECURSE ${PN}_${TN}_src_platform ${DIR_PKG}drivers/${system}/*.c)\n"
		    "list(APPEND ${PN}_${TN}_src ${${PN}_${TN}_src_platform})\n"
		    "add_library(${PN}_${TN} OBJECT ${${PN}_${TN}_src})\n"
		    "if (CMAKE_C_COMPILER_ID MATCHES \"GNU|Clang\")\n"
		    "\ttarget_compile_options(${PN}_${TN} PRIVATE\n"
...
TEST(gen_cmake_pkg_drv_out)
{
	START;

	t_gen_common_t com = {0};
	EXPECT_EQ(t_gen_pkg_drv_out(&com, STRV("C")), 0);
...
TEST(gen_cmake_pkg_test)
{
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "list(FILTER ${PN}_${TN}_src EXCLUDE REGEX \"test/(windows|linux)/.*\\\\.c$\")\n"
		    "file(GLOB_RECURSE ${PN}_${TN}_src_platform ${DIR_PKG}test/${system}/*.c)\n"
		    "list(APPEND ${PN}_${TN}_src ${${PN}_${TN}_src_platform})\n"
		    "add_executable(${PN}_${TN} ${${PN}_${TN}_src})\n"
		    "if (CMAKE_C_COMPILER_ID MATCHES \"GNU|Clang\")\n"
		    "\ttarget_compile_options(${PN}_${TN} PRIVATE\n"
...
TEST(gen_cmake_pkg_depends)
{
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "\t)\n"
		    "endif()\n"
		    "target_link_libraries(${PN}_${TN} PRIVATE lib_)\n"
		    "set_target_properties(${PN}_${TN} PROPERTIES\n"
		    "\tOUTPUT_NAME \"${PN}\"\n"
		    "\tRUNTIME_OUTPUT_DIRECTORY bin/\n"
		    "\tRUNTIME_OUTPUT_DIRECTORY_DEBUG bin/\n"
...
TEST(gen_cmake_pkg_rdepends)
{
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "\t)\n"
		    "endif()\n"
		    "target_link_libraries(${PN}_${TN} PUBLIC base_)\n"
		    "set_target_properties(${PN}_${TN} PROPERTIES\n"
		    "\tOUTPUT_NAME \"${PN}\"\n"
		    "\tARCHIVE_OUTPUT_DIRECTORY lib/\n"
		    "\tARCHIVE_OUTPUT_DIRECTORY_DEBUG lib/\n"
	...
	EXPECT_STRN(tmp.data,
		    ...
		    "\t)\n"
		    "endif()\n"
		    "target_link_libraries(${PN}_${TN} PRIVATE lib1_ lib2_)\n"
		    "set_target_properties(${PN}_${TN} PROPERTIES\n"
		    "\tOUTPUT_NAME \"${PN}\"\n"
		    "\tRUNTIME_OUTPUT_DIRECTORY bin/\n"
		    "\tRUNTIME_OUTPUT_DIRECTORY_DEBUG bin/\n"
