...
TEST(config_fs_pkgs_queue)
{
	...
	EXPECT_EQ(config_fs(&config, &tmp, &schema, &registry, &fs, &proc, STRV_NULL, STRV_NULL, STRV_NULL, &buf, ALLOC_STD, DST_NONE()),
		  0);
...
TEST(config_fs_ext_sync_queue_oom)
{
	...
	log_set_quiet(0, 1);
	int ret = config_fs(&config, &tmp, &schema, &registry, &fs, NULL, STRV_NULL, STRV(cur_path), STRV("proj"), &buf, alloc, DST_NONE());
	log_set_quiet(0, 0);
...
TEST(config_fs_ext_sync)
{
	...
	EXPECT_EQ(config_fs(&config, &tmp, &schema, &registry, &fs, &proc, STRV_NULL, STRV_NULL, STRV("proj"), &buf, ALLOC_STD, DST_NONE()),
		  0);
...
TEST(config_fs_invalid_work_kind)
{
	...
	EXPECT_NE(mod, NULL);
	int (*config_fs_fn)(mod_t *, config_t *, config_t *, const config_schema_t *, registry_t *, config_sync_plan_t *, fs_t *, strv_t, strv_t, strv_t,
			    str_t *, alloc_t, dst_t) = mod->config_fs;
	mod->config_fs = invalid_kind_config_fs;

	char storage[64] = {0};
	str_t buf	 = STRB(storage, 0);

	EXPECT_EQ(config_fs(&config, &tmp, &schema, &registry, &fs, NULL, STRV_NULL, STRV_NULL, STRV("proj"), &buf, ALLOC_STD, DST_NONE()), 1);
