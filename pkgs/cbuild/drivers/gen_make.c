...

{
	...
	make_var_add_val(make, act, MSTR(strvbuf_get(&proj->strs, pkg->name)));
	...
	if (uri.len > 0) {
		...
		strv_t uri_file = strvbuf_get(&proj->strs, pkg->uri_name);
		buf->len	= 0;
		...
		strv_t uri_root = strvbuf_get(&proj->strs, pkg->uri_dir);
		if (uri_root.len > 0) {
	...
	list_foreach(&proj->targets, i, target)
	{
		...
		make_var_add_val(make, act, MSTR(strvbuf_get(&proj->strs, target->name)));
		make_inc_add_act(make, inc, act);

		make_var(make, STRV("$(PN).$(TN).HEADERS"), MAKE_VAR_INST, &act);
		strv_t include = strvbuf_get(&proj->strs, pkg->inc);
		if (include.len > 0) {
		...
		case TARGET_TYPE_TST: {
			...
			if (deps->cnt > 0) {
				...
				arr_foreach(deps, i, dep)
				{
					...
					str_cat(buf, strvbuf_get(&proj->strs, dpkg->name));
					str_cat(buf, STRV("."));
					str_cat(buf, strvbuf_get(&proj->strs, dtarget->name));
					str_cat(buf, STRV(")"));
...
static int gen_make(const gen_driver_t *drv, const proj_t *proj, strv_t proj_dir, strv_t build_dir)
{
	...
	strv_t poutdir = strvbuf_get(&proj->strs, proj->outdir);
	...
	{
		if (strvbuf_get(&proj->strs, pkg->uri_str).len > 0) {
	...
	if (types[TARGET_TYPE_LIB]) {
		...
		make_rule_add_depend(&make, def_rule_obj, MRULE(MVAR(pkgfiles[PKGSRC_H].var)));
		make_rule_add_depend(&make, def_rule_obj, MRULE(MSTR(STRV("$($(PN).$(TN).HEADERS)"))));
		make_rule_add_depend(&make, def_rule_obj, MRULE(MSTR(STRV("$($(PN).ROOT)"))));
		make_cmd(&make, MCMD(STRV("@mkdir -pv $$(@D)")), &act);
		make_rule_add_act(&make, def_rule_obj, act);
		make_cmd(&make,
			 MCMD(STRV("$(TCC) -m$(BITS) -c $(PKGDIR_SRC:%=-I%) $($(PN).$(TN).INCLUDES:%=-I%) $(CFLAGS) -o $$@ $$<")),
	...
	if (proj->pkgs.cnt > 0) {
		...
		arr_foreach(&order, i, id)
		{
			...
			str_cat(&buf, strvbuf_get(&proj->strs, pkg->path));
