...
static int mod_drv_config_fs(mod_t *mod, config_t *config, config_t *tmp, const config_schema_t *schema, registry_t *registry, fs_t *fs,
			     proc_t *proc, strv_t proj_path, strv_t cur_path, strv_t name, str_t *buf, alloc_t alloc, dst_t dst)
{
	...
	if (fs_isdir(fs, STRVS(path))) {
		...
		config_str_list(tmp, CONFIG_TGTS, pkg, tgt, CONFIG_ACT_APP, STRV("drivers"), NULL);
		config_int(tmp, CONFIG_TGT_TYPE, pkg, tgt, CONFIG_ACT_SET, TARGET_TYPE_DRV);
		config_str(tmp, CONFIG_TGT_SRC, pkg, tgt, CONFIG_ACT_SET, STRV("drivers"));
		config_str_list(tmp, CONFIG_TGT_INCS_PRIV, pkg, tgt, CONFIG_ACT_SET, STRV("drivers"), NULL);
