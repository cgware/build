...
static int get_target_deps(const targets_t *targets, uint id, arr_t *arr)
{
	int ret = 0;

	const target_t *target = arr_get(&targets->targets, id);
...
static int get_pkg_targets(const pkgs_t *pkgs, const targets_t *targets, uint id, arr_t *arr)
{
	...
	{
		get_target_deps(targets, target->pkg, arr);
