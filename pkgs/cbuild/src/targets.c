...
targets_t *targets_init(targets_t *targets, uint targets_cap, alloc_t alloc)
{
	...
	if (strbuf_init(&targets->names, targets_cap, targets_cap * 8, alloc) == NULL ||
	    ...
	    arr_init(&targets->targets, targets_cap, sizeof(target_t), alloc) == NULL ||
...
void targets_free(targets_t *targets)
{
	...
	target_t *target;
	arr_foreach(&targets->targets, target)
	{
		target_free(target);
	}

	arr_free(&targets->targets);
...
target_t *targets_add(targets_t *targets, lnode_t *list, strv_t name, lnode_t *id)
{
	...
	uint index;
	if (strbuf_get_index(&targets->names, name, &index) == 0) {
		if (id) {
			*id = index;
		}
		list_set_next(&targets->targets, *list, index);
...
target_t *targets_add_dep(targets_t *targets, lnode_t id, strv_t dep)
{
	...
	if (strbuf_get_index(&targets->names, dep, &dep_id) && targets_add(targets, &list, dep, &dep_id) == NULL) {
		return NULL;
	}

	target_t *target = arr_get(&targets->targets, id);
	if (target == NULL) {
		log_error("build", "targets", NULL, "target is not in dependecy list");
		return NULL;
	}
	...
	if (data == NULL) {
		log_error("build", "targets", NULL, "failed to add target dependecy");
		return NULL;
	}
