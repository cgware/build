...
static int on_pkg(path_t *path, const char *folder, void *priv)
{
	...
	uint id;
	if (pkgs_add_pkg(p->pkgs, name, &id) == NULL) {
		return 1; // LCOV_EXCL_LINE
	}

	return pkg_load(id, dir, p->pkgs, p->targets, p->pkgs->alloc);
}
...
int proj_set_dir(proj_t *proj, strv_t dir)
{
	...
	if (is_pkgs) {
		...
		ret |= files_foreach(&tmp, on_pkg, NULL, &priv);
	}
